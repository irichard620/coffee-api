...
function getIngredientDoc(ingredientModel, version) {
  ...
  if (!version || version === "1.0.0") {
    if (dbDoc.amount_type === "") {
      ...
      dbDoc.amount_type === "Milliliter" ||
      dbDoc.amount_type === "Centiliter"
    ) {
      ...
      dbDoc.fractional_amount = getFractionFromRoundedDecimal(
        splitConvertedDecimalToEighth[1]
      );
...
function getMixxyRecipeDoc(recipeModel, version) {
  ...
  for (let i = 0; i < recipeModel.ingredients.length; i += 1) {
    ingredientsToAdd.push(
      getIngredientDoc(recipeModel.ingredients[i], version)
    );
  }
